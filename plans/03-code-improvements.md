...
- [ ] **D4.** Found by Plan 4: `@rockpack/utils` and `@rockpack/tsconfig` publish no README, and `@rockpack/codestyle` exports its internal `isString`. _Partly done 2026-09-24: `@rockpack/utils` and `@rockpack/tsconfig` have READMEs; the internal `isString` export of codestyle is still open (S-step)._
