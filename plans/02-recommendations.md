...
- [x] **A1. CI workflow.** Add `.github/workflows/ci.yml` triggered on `push` to `next` and `master` and on `pull_request`. _Done 2026-09-24: `.github/workflows/ci.yml` (check, e2e-light, examples-build, nightly/dispatch starter-e2e); all jobs pass locally; `e2e/compiler-e2e` joins e2e-light when Plan 4 adds it._
  Jobs:
...
- [ ] **A6. Make `e2e/starter-e2e` deterministic.** The full rewrite is specified in [Plan 4, section 4](./04-e2e.md); the list below is the minimum set of fixes and stays here for traceability. Note that Plan 4 keeps `--mode=test` resolving current versions (its `latest` mode depends on it); the "no network" requirement applies to Plan 4 `pinned` mode only.
...
- [ ] **H6. Dead compiler modules.** `compiler/src/utils/common-multi-validators.ts` (`commonMultiValidator`) and `compiler/src/utils/find-html.ts` (`findHtml`) are neither imported nor exported. Found in Plan 1 Phase 6.4, which covers them with small specs so coverage stays honest. Delete both modules together with their specs and the now-unused `errors/markup-compiler.ts` messages, or wire them in if a markup compiler is planned.
