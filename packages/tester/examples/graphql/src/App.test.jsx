...
describe('Test queries', () => {
    test('GET_BOOKS - get the array of books with the book It', () => {
        expect(document.getElementById('books').childNodes[0].innerHTML).toBe('It');
    });
    ...
    test('BOOK_READ - set the book It as readed', async (next) => {
        ...
        expect(getComputedStyle(it, null)['text-decoration']).toBe('line-through');
        next();
    });
    ...
    test('GET_BOOK - get the favorite book Jurassic Park', async (next) => {
        ...
        let favorite = document.getElementsByTagName('h2')[0].nextSibling;
        expect(favorite.innerHTML).toBe('Jurassic Park');
        next();
    });
});
